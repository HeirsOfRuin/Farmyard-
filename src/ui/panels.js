...
export function renderPlan(state, draft) {
  ...
  out.push(
    `<div style="font-size:.76rem;color:var(--ink-3);margin:-2px 0 6px">${bottleneckNote} ` +
  ...
  // labourShortage was declared on every war-era history event since the game
  // shipped and read by nothing — a world war changed the story text and
  // nothing else. It now actually thins the family workforce (see
  // labourForce() in derive.js); this is the only place that says why, so a
  // sudden drop in "Hands available" is not mistaken for a bug.
  const atWar = (state.activeEffects || []).some((e) => e.effects?.warYears && e.expiresAfter >= state.year);
  if (state.modifiers?.labourShortage > 0) {
    const shortagePct = Math.round(Math.min(state.modifiers.labourShortage, 0.85) * 100);
    out.push(
      `<div style="font-size:.78rem;color:var(--alarm);margin:-2px 0 6px">` +
        `${atWar ? 'The war' : 'Trouble in the district'} is costing the farm's own family labour about ` +
        `${shortagePct}% this year — hired hands are not affected and can make up the gap.</div>`
    );
  }
